...
const AccountModal = ({ isOpen, onClose, userSkillLevel, onSkillLevelChange, onResearchInterestsChange }) => {
  ...
  const { isDarkMode, toggleDarkMode, syncModeFromProfile, setCurrentUserId } = useTheme();
        ...
        if (mounted) {
          if (session?.user) {
            ...
            try {
              ...
              // Don't fail loading if profile fails
            }
          } else {
            setUser(null);
            setCurrentUserId?.(null);
          ...
          if (event === 'SIGNED_IN' && session?.user) {
            ...
            try {
              ...
              if (profile && mounted) {
                ...
                // Sync dark mode preference from profile after sign in
                if (profile.mode && syncModeFromProfile) {
                  syncModeFromProfile(profile.mode);
                }
              }
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
        <div className={`w-full max-w-5xl h-[85vh] sm:h-[88vh] rounded-2xl shadow-2xl border flex flex-col lg:flex-row overflow-hidden ${
          ...
          <div className={`lg:hidden border-b p-4 flex-shrink-0 ${
            ...
            {/* Mobile Navigation */}
            <nav className={`flex space-x-1 rounded-xl p-1 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
              {tabs.map(tab => {
                const Icon = tab.icon;
            ...
            <div className={`mt-3 flex items-center justify-between px-3 py-2.5 rounded-xl border ${
